...
struct sequencia{
  ...
  double sd; // mean deleterius exp distributed
  ...
  double samedio; // efeito medio das beneficas
};
  ...
  int main(int ac, char **av)
{
 ...
 double nmutdel,nmutben, nmutdel1,nmutben1,*auxFM,*auxFM1,MIG;
  ...
  while( (++conf)<=config )
    {  
      fitnessinit(&sequence,Npop);fitnessinit(&sequence1,Npop);
        for(i=0;i<Npop;i++)
      ...
      while( t<tmax )
	{
      ...
      mutationselection(&sequence1,Npop,t,Genomesite);
        ...
        if((t%TSample)==0){
            ...
            sprintf(arq2,"SocialS1500freq-N%d-Ud%lf-Ua%lf-Sd%lf-Sa%lf-MIG%lf.dat",Npop,sequence.U,sequence.Ub,sequence.sd,sequence.samedio,MIG);
            ptt2 = fopen(arq2,"a");
            fprintf(ptt2," \n %d \t", t);
  ...
  //      {
            ...
            if(sequence.Ftot>0)
            {
                sprintf(arq1,"SocialS1500res-N%d-Ud%lf-Ua%lf-Sd%lf-Sa%lf.dat",Npop,sequence.U,sequence.Ub,sequence.sd,sequence.samedio);
                ptt1 = fopen(arq1,"a");
