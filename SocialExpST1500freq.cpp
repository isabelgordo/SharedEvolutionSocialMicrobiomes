// This code simulates the accumulation of deleterious and beneficial mutations in a clonal population of fixed size Npop, composed of two demes
// Deleterious mutations occur at a rate U, beneficial mutations at a rate Ub
// the effects of beneficial mutations, as well as of deleterious mutations, are taken from a Exponential distribution
// the number of mutations, the mean fitness of the population, and the frequency of each beneficial mutation
// are recorded in a file after a given number of generations Tsample
...
//
// to compile the code type g++ SocialExpST1500freq.cpp -o SocialExpST1500freq -lm
// to run the code type ./SocialExpST400 seed Npop Ud Ub meanSd meanSa nmig
...
struct sequencia{
  double U; // rate for deleterious mutations
  double Ub; //rate for beneficial mutations
  double sd; // mean deleterius exponentially distributed
  double *F; // vector of fitnesses
  int *NMUTDEL; //number of deleterious mutations
  int *NMUTBEN; //number of beneficial mutations
  int **SITEBEN; //matrix of beneficial mutations sites
  double Ftot; // mean population fitness
  double samedio;
    double *sa; // vector de efeito das beneficas from an exponential each site is assigned an effect
};
      ...
      while( t<tmax )
	{
      ...
      mutationselection(&sequence1,Npop,t,Genomesite);
        ...
        if((t%TSample)==0){
            ...
            sprintf(arq2,"SocialExpS1500f-N%d-Ud%lf-Ua%lf-Sd%lf-Sa%lf-MIG%lf.txt",Npop,sequence.U,sequence.Ub,sequence.sd,sequence.samedio,MIG);
            ptt2 = fopen(arq2,"a");
            fprintf(ptt2," \n %d \t", t);
            ...
            if(sequence.Ftot>0)
            {
                sprintf(arq1,"SocialExpS1500res-N%d-Ud%lf-Ua%lf-Sd%lf-Sa%lf.txt",Npop,sequence.U,sequence.Ub,sequence.sd,sequence.samedio);
                ptt1 = fopen(arq1,"a");
        ...
        // make the first nmig individulas migrate from deme1 to deme2 and vice versa
        ...
        // copy info of migrants from deme2
        ...
        // replace info of migrants from deme1 to deme2
        ...
        // replace info of migrants from deme2 to deme1
