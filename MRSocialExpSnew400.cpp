  ...
  int main(int ac, char **av)
{
 ...
 double nmutdel,nmutben, nmutdel1,nmutben1,*auxFM,*auxFM1,MIG,r,saaux;
  ...
  while( (++conf)<=config )
    ...
    //       for( k=0; k<Genomesite; k++ ) printf(" Sainit %lf \t", sequence.sa[k]);
    //    printf(" \n");
  ...
  //     printf("before mutsel %d \t",t);
    ...
    //            printf("\n %d \t",Npop+k);
     //           for(i=1;i<=Genomesite;i++) printf(" %d \t", sequence1.SITEBEN[k][i]);
            ...
            sprintf(arq2,"Social400ExpSf-N%d-Ud%lf-Ua%lf-Sd%lf-Sa%lf-MIG%lf.dat",Npop,sequence.U,sequence.Ub,sequence.sd,sequence.samedio,MIG);
            ptt2 = fopen(arq2,"a");
            fprintf(ptt2," \n %d \t", t);
  ...
  //      {
            ...
            if(sequence.Ftot>0)
            {
                sprintf(arq1,"Social400ExpSres-N%d-Ud%lf-Ua%lf-Sd%lf-Sa%lf.dat",Npop,sequence.U,sequence.Ub,sequence.sd,sequence.samedio);
                ptt1 = fopen(arq1,"a");
